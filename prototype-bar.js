...
(function () {
  "use strict";
  var VERSION = "3.0.0"; /* stamped by release.sh; compared with the published version.json */
  ...
  /* The chosen start, applied by the bar itself: opening the prototype's ENTRY
     page (its index, or the page named by `start`) hands you the screen whose
     switch is on. Never on another page — a redirect on every reload would
      ...
      html: function () {
        ...
        return '<div class="pbar-menu-head">Where the prototype opens</div>' +
          ...
          toggle(!!sp, "data-start-here", sp ? "Starts on a chosen page" : "Start on the page I'm on",
            sp ? "Opening the prototype takes you to " + sp + ". Turn off to go back." : "Remembers this page as the start, in this browser, without registering it. Opening the prototype's first page then takes you here.");
      },
      ...
      html: function () {
        ...
        if (!live) {
          return '<div class="pbar-menu-head pbar-share-head">Share</div>' +
            ...
            item("is-primary", "data-copy", "Copy link");
        }
        return '<div class="pbar-menu-head pbar-share-head">Share</div>' +
